...
import {
  ...
  setEdgeTradingStop,
  getAllEnabledEdgeTrading,
  setEdgeTradingLastEntered,
  hasEnteredSlug,
  recordEnteredSlug,
  listEdgeTradingEntriesByUserId,
  ensureEdgeTradingEnteredSlugsTable,
  ensureEdgeTradingMarketsColumn,
} from './db/edgeTrading.js'
...
import {
  upsertClobTrades,
  listClobTradesByUserId,
  getDistinctTokenIdsByUserId,
  getTokenIdToMarketSlug,
  ensureUserClobTradesTable,
} from './db/userClobTrades.js'
import {
  ensureCopyTradingTables,
  upsertCopySubscription,
  disableCopySubscription,
  getEnabledCopySubscriptions,
  listCopySubscriptionsByUser,
  updateLastSeenTimestamp,
  isTxHashCopied,
  recordCopyTrade,
  listCopyHistoryByUser,
  getCopyPositionShares,
  type CopySubscriptionRow,
} from './db/copyTrading.js'

const port = parseInt(process.env.API_PORT ?? '3001', 10)

type AuthPayload = { userId: number; walletId: number; address: string }
...
function requireAuth(
  ...
  try {
    const payload = jwt.verify(auth.slice(7), config.jwtSecret) as AuthPayload
...
function formatPositionForResponse(row: Record<string, unknown>): Record<string, unknown> {
  const exitReason = row.exit_reason != null ? String(row.exit_reason) : null
  const status = row.status != null ? String(row.status) : ''
  let outcome: 'open' | 'won' | 'lost' | 'closed' = 'closed'
  if (status === 'open' || status === 'closing') outcome = 'open'
  else if (exitReason === 'profit') outcome = 'won'
  else if (exitReason === 'stoploss') outcome = 'lost'
  else outcome = 'closed' // time, manual, or null
...
function formatCopySubscription(row: CopySubscriptionRow): Record<string, unknown> {
  return {
    id: Number(row.id),
...
function formatStrategyForResponse(row: StrategyRow | Record<string, unknown>): Record<string, unknown> {
  const n = (v: unknown): number | null => (v == null ? null : Number(v))
  const num = (v: unknown): number => (v == null ? 0 : Number(v))
  return {
    id: num((row as Record<string, unknown>).id),
    user_id: num((row as Record<string, unknown>).user_id),
    wallet_id: num((row as Record<string, unknown>).wallet_id),
    name: (row as Record<string, unknown>).name,
    market: (row as Record<string, unknown>).market,
    active: Boolean((row as Record<string, unknown>).active),
    entry_side: (row as Record<string, unknown>).entry_side,
    entry_odd_max: (row as Record<string, unknown>).entry_odd_max != null ? String((row as Record<string, unknown>).entry_odd_max) : null,
    entry_seconds_to_expiry_min: num((row as Record<string, unknown>).entry_seconds_to_expiry_min),
    entry_odd_change_window_s: n((row as Record<string, unknown>).entry_odd_change_window_s),
    entry_odd_change_min: (row as Record<string, unknown>).entry_odd_change_min != null ? String((row as Record<string, unknown>).entry_odd_change_min) : null,
    entry_odd_change_pct_min: (row as Record<string, unknown>).entry_odd_change_pct_min != null ? String((row as Record<string, unknown>).entry_odd_change_pct_min) : null,
    exit_stop_loss: (row as Record<string, unknown>).exit_stop_loss != null ? String((row as Record<string, unknown>).exit_stop_loss) : null,
    exit_stop_loss_pct: (row as Record<string, unknown>).exit_stop_loss_pct != null ? String((row as Record<string, unknown>).exit_stop_loss_pct) : null,
    exit_seconds_to_expiry_max: num((row as Record<string, unknown>).exit_seconds_to_expiry_max),
    exit_profit_odd: (row as Record<string, unknown>).exit_profit_odd != null ? String((row as Record<string, unknown>).exit_profit_odd) : null,
    exit_profit_pct: (row as Record<string, unknown>).exit_profit_pct != null ? String((row as Record<string, unknown>).exit_profit_pct) : null,
    order_size_usd: (row as Record<string, unknown>).order_size_usd != null ? String((row as Record<string, unknown>).order_size_usd) : '0',
...
async function main() {
  ...
  app.get('/edge-trading/status', async (req, reply) => {
    ...
    if (!row) return { enabled: false, orderSizeUsd: null, lastEnteredSlug: null, lastEnteredAt: null, markets: null }
    return {
  ...
  app.post('/edge-trading/start', async (req, reply) => {
    const auth = requireAuth(req, reply)
    if (!auth) return
    const body = (req.body ?? {}) as { orderSizeUsd?: number; markets?: string[] }
  ...
  app.post('/auth/login', async (req, reply) => {
    ...
    if (!utils.isAddress(body.address)) return reply.code(400).send({ error: 'invalid address' })
    ...
    const payload: AuthPayload = { userId: user.id, walletId: wallet.id, address: normalized }
  ...
  app.post('/wallets/custodial', async (req, reply) => {
    ...
    if (auth.userId !== body.userId) return reply.code(403).send({ error: 'forbidden' })
    const w = Wallet.createRandom()
  ...
  app.post('/wallets/:walletId/clob/derive', async (req, reply) => {
    const auth = requireAuth(req, reply)
    if (!auth) return
    const walletId = parseInt((req.params as { walletId: string }).walletId, 10)
    if (!Number.isFinite(walletId)) return reply.code(400).send({ error: 'invalid walletId' })
    if (auth.walletId !== walletId) return reply.code(403).send({ error: 'forbidden' })
    const wallet = await getWalletById(walletId)
    if (!wallet) return reply.code(404).send({ error: 'wallet not found' })
    if (wallet.type !== 'custodial') return reply.code(400).send({ error: 'wallet must be custodial' })
    if (!wallet.encrypted_private_key) return reply.code(400).send({ error: 'wallet missing private key' })
  ...
  app.get('/wallets/:walletId/reveal-key', async (req, reply) => {
    ...
    if (!auth) return
    const walletId = parseInt((req.params as { walletId: string }).walletId, 10)
    if (!Number.isFinite(walletId) || auth.walletId !== walletId) return reply.code(403).send({ error: 'forbidden' })
    const confirm = (req.query as { confirm?: string }).confirm
    if (confirm !== 'yes') return reply.code(400).send({ error: 'Add ?confirm=yes to reveal the private key. Never share it.' })
    const wallet = await getWalletById(walletId)
    if (!wallet) return reply.code(404).send({ error: 'wallet not found' })
    if (!wallet.encrypted_private_key) return reply.code(400).send({ error: 'wallet missing private key' })
    const privateKey = decryptString(wallet.encrypted_private_key)
  ...
  app.post('/wallets/:walletId/approve-usdc', async (req, reply) => {
    ...
    if (!auth) return
    const walletId = auth.walletId
    const body = (req.body ?? {}) as { amountUsdc?: number }
    ...
    const txHash =
      ...
      (response as unknown as { transactionHash?: string; hash?: string }).transactionHash ??
      (response as unknown as { hash?: string }).hash
  ...
  app.post('/wallets/:walletId/withdraw-usdc', async (req, reply) => {
    ...
    if (!auth) return
    const walletId = auth.walletId
    const body = (req.body ?? {}) as { toAddress?: string; amountUsdc?: number }
    if (!body.toAddress) return reply.code(400).send({ error: 'toAddress required' })
    if (body.amountUsdc == null || body.amountUsdc <= 0) {
      return reply.code(400).send({ error: 'amountUsdc must be > 0' })
    }
    ...
    try {
      if (!utils.isAddress(body.toAddress)) return reply.code(400).send({ error: 'invalid toAddress' })
    ...
    let proxyAddress: string | null = wallet.builder_proxy_address ?? null
    if (!proxyAddress) proxyAddress = await ensureBuilderProxyForWallet(walletId)
    ...
    try {
      if (!hasSafeBal) throw new Error('no Safe balance — use EOA fallback')
      const relayerUrl = process.env.POLY_BUILDER_RELAYER_URL ?? 'https://relayer-v2.polymarket.com'
      const builderConfig = new BuilderConfig({
        localBuilderCreds: {
          key: config.polyBuilderApiKey,
      ...
      txHash =
        result?.transactionHash ??
        (response as unknown as { transactionHash?: string }).transactionHash ??
        (response as unknown as { hash?: string }).hash
    } catch {
  ...
  app.post('/strategies', async (req, reply) => {
    ...
    const walletId = auth.walletId
    if (!body.name || !body.market) return reply.code(400).send({ error: 'name and market required' })
    if (!body.entrySide) return reply.code(400).send({ error: 'entrySide required' })
    if (body.orderSizeUsd == null) return reply.code(400).send({ error: 'orderSizeUsd required' })
    ...
    if (body.entryOddMax == null) return reply.code(400).send({ error: 'entryOddMax required' })
    ...
    // Range checks
    if (body.orderSizeUsd <= 0) return reply.code(400).send({ error: 'orderSizeUsd must be > 0' })
  ...
  app.get('/users/:userId/strategies', async (req, reply) => {
    ...
    if (!Number.isFinite(userId)) return reply.code(400).send({ error: 'invalid userId' })
    if (auth.userId !== userId) return reply.code(403).send({ error: 'forbidden' })
  ...
  function strategyAllowed(
    strategy: { user_id: number | string; wallet_id: number | string },
    auth: AuthPayload
  ): boolean {
  ...
  const strategyForbiddenMessage =
    'This strategy is tied to a different sign-in account. Use the same MetaMask address you used when you created it.'

  app.get('/strategies/:strategyId', async (req, reply) => {
    const auth = requireAuth(req, reply)
    if (!auth) return
    const strategyId = parseInt((req.params as { strategyId: string }).strategyId, 10)
    if (!Number.isFinite(strategyId)) return reply.code(400).send({ error: 'invalid strategyId' })
    const strategy = await getStrategyById(strategyId)
    if (!strategy) return reply.code(404).send({ error: 'strategy not found' })
    if (!strategyAllowed(strategy, auth)) return reply.code(403).send({ error: strategyForbiddenMessage })
  ...
  app.patch('/strategies/:strategyId', async (req, reply) => {
    ...
    if (!Number.isFinite(strategyId)) return reply.code(400).send({ error: 'invalid strategyId' })
    {
      ...
      if (!strategyAllowed(existing, auth)) return reply.code(403).send({ error: strategyForbiddenMessage })
    }
    ...
    if (body.market != null) fields.market = body.market
    if (body.entrySide != null) fields.entry_side = body.entrySide
    if ('entryOddMax' in body) fields.entry_odd_max = body.entryOddMax ?? null
    if (body.entrySecondsToExpiryMin != null) fields.entry_seconds_to_expiry_min = body.entrySecondsToExpiryMin
    if ('entryOddChangeWindowS' in body) fields.entry_odd_change_window_s = body.entryOddChangeWindowS ?? null
    if ('entryOddChangeMin' in body) fields.entry_odd_change_min = body.entryOddChangeMin ?? null
    if ('entryOddChangePctMin' in body) fields.entry_odd_change_pct_min = body.entryOddChangePctMin ?? null
    if ('exitStopLoss' in body) fields.exit_stop_loss = body.exitStopLoss ?? null
    if ('exitStopLossPct' in body) fields.exit_stop_loss_pct = body.exitStopLossPct ?? null
    if (body.exitSecondsToExpiryMax != null) fields.exit_seconds_to_expiry_max = body.exitSecondsToExpiryMax
    if ('exitProfitOdd' in body) fields.exit_profit_odd = body.exitProfitOdd ?? null
    if ('exitProfitPct' in body) fields.exit_profit_pct = body.exitProfitPct ?? null
    if (body.orderSizeUsd != null) fields.order_size_usd = body.orderSizeUsd
    ...
    if (body.active === true) {
      ...
      if (!wallet) return reply.code(400).send({ error: 'wallet not found for strategy' })
      if (wallet.type !== 'custodial') return reply.code(400).send({ error: 'wallet must be custodial' })
  ...
  app.get('/strategies/:strategyId/positions', async (req, reply) => {
    ...
    if (!strategy) return reply.code(404).send({ error: 'strategy not found' })
    if (!strategyAllowed(strategy, auth)) return reply.code(403).send({ error: strategyForbiddenMessage })
    const limitQ = (req.query as { limit?: string }).limit
    const limit = Math.min(Math.max(parseInt(limitQ ?? '20', 10) || 20, 1), 200)
    const rows = await listPositionsForStrategy(strategyId, limit)
    const positions = rows.map((r) => formatPositionForResponse(r as unknown as Record<string, unknown>))
    return { positions }
  })
  ...
  app.get('/clob/trades', async (req, reply) => {
    const auth = requireAuth(req, reply)
    if (!auth) return
    const limit = Math.min(Math.max(parseInt((req.query as { limit?: string }).limit ?? '100', 10) || 100, 1), 500)
    const wallet = await getWalletById(auth.walletId)
    ...
    if (tradingAddress) {
      try {
        const res = await fetch(
          `https://data-api.polymarket.com/trades?user=${encodeURIComponent(tradingAddress)}&limit=200&takerOnly=false`,
        ...
        console.error('[CLOB] Data API trades failed', { userId: auth.userId, err: (err as Error)?.message })
      }
    }
    ...
    if (!synced) {
      try {
        ...
        const rawTrades = await (client.getTrades as (p: Record<string, unknown>, taker: boolean) => Promise<unknown>)({}, true)
        const arr = Array.isArray(rawTrades) ? (rawTrades as unknown[]) : []
        const asRecords = arr.map((t) => (typeof t === 'object' && t !== null ? (t as Record<string, unknown>) : { id: String(t) }))
        await upsertClobTrades(auth.walletId, auth.userId, asRecords)
      } catch (err) {
        console.error('[CLOB] getTrades failed', { userId: auth.userId, err: (err as Error)?.message })
      }
    }
  ...
  app.get('/clob/orders', async (req, reply) => {
    ...
    try {
      ...
      console.error('[CLOB] getOpenOrders failed', { userId: auth.userId, err: (err as Error)?.message })
  ...
  app.get('/clob/positions', async (req, reply) => {
    ...
    try {
      const { client } = await getClobClientForWallet(auth.walletId)
      const positions: { tokenId: string; balance: number; marketSlug: string | null }[] = []
      ...
      console.error('[CLOB] positions failed', { userId: auth.userId, err: (err as Error)?.message })
  ...
  app.get('/dashboard', async (req, reply) => {
    ...
    const [
      ...
      (async (): Promise<string | null> => {
        if (!tradingAddress || !utils.isAddress(tradingAddress)) return null
        const USDC_E = '0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174'
    ...
    const usdcParsed = usdcRaw ? (() => { try { return JSON.parse(usdcRaw) as { balance: string; allowance: string } } catch { return null } })() : null

    // Build position lookup by strategy
    const positionsByStrategy = new Map<number, typeof allPositions[0][]>()
    for (const pos of allPositions) {
      const sid = Number(pos.strategy_id)
      if (!positionsByStrategy.has(sid)) positionsByStrategy.set(sid, [])
      positionsByStrategy.get(sid)!.push(pos)
    }

    const toIso = (v: unknown) => (v instanceof Date ? v.toISOString() : v != null ? String(v) : null)

    const formatPos = (p: typeof allPositions[0]) => {
    ...
    const leaderStats: Record<string, { executed: number; failed: number; skipped: number; totalUsd: number }> = {}
    for (const h of copyHistory) {
      const addr = h.leader_address
      if (!leaderStats[addr]) leaderStats[addr] = { executed: 0, failed: 0, skipped: 0, totalUsd: 0 }
      leaderStats[addr][h.status as 'executed' | 'failed' | 'skipped']++
      if (h.status === 'executed' && h.amount_usd != null) leaderStats[addr].totalUsd += Number(h.amount_usd)
    }
  ...
  app.get('/copy-trading/user', async (req, reply) => {
    ...
    const fetchAllTrades = async (): Promise<unknown[]> => {
      const PAGE_SIZE = 500
      const all: unknown[] = []
      let offset = 0
      for (;;) {
  ...
  app.post('/copy-trading/unsubscribe', async (req, reply) => {
    const auth = requireAuth(req, reply)
    if (!auth) return
    const body = (req.body ?? {}) as { leaderAddress?: string }
    if (!body.leaderAddress) return reply.code(400).send({ error: 'leaderAddress required' })
  ...
  await ensureEdgeTradingEnteredSlugsTable().catch((err) =>
    console.error('[Server] ensureEdgeTradingEnteredSlugsTable failed', (err as Error)?.message)
  )
  await ensureEdgeTradingMarketsColumn().catch((err) =>
    console.error('[Server] ensureEdgeTradingMarketsColumn failed', (err as Error)?.message)
  )
  await ensureUserClobTradesTable().catch((err) =>
    console.error('[Server] ensureUserClobTradesTable failed', (err as Error)?.message)
  )
  await ensureCopyTradingTables().catch((err) =>
    console.error('[Server] ensureCopyTradingTables failed', (err as Error)?.message)
  )
...
async function runEdgeTradingLoop(): Promise<void> {
  ...
  for (const { market, asset, horizon } of EDGE_MARKETS) {
    ...
    if (insight.slug !== resolvedSlug) continue

    const sampleTs = insight.sample_ts ? new Date(insight.sample_ts).getTime() : 0
    if (Number.isNaN(sampleTs) || Date.now() - sampleTs > EDGE_INSIGHT_MAX_AGE_MS) continue

    const synthUp = insight.synth_probability_up ?? NaN
    const polyUp = insight.polymarket_probability_up ?? NaN
    if (!Number.isFinite(synthUp) || !Number.isFinite(polyUp)) continue
    const edgePp = (synthUp - polyUp) * 100
    let side: 'up' | 'down' | null = null
    if (edgePp >= EDGE_ENTRY_THRESHOLD_UP) side = 'up'
    else if (edgePp <= EDGE_ENTRY_THRESHOLD_DOWN) side = 'down'
    ...
    if (side === 'up' && outcome !== 'up') continue
    if (side === 'down' && outcome !== 'down') continue
    ...
    if (!Number.isFinite(entryOdd) || entryOdd < EDGE_MIN_ENTRY_ODD || entryOdd > EDGE_MAX_ENTRY_ODD) continue

    let info: Awaited<ReturnType<typeof fetchMarketInfo>>
    try {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        const resp = (await client.createAndPostMarketOrder(
          { tokenID, side: Side.BUY, amount: orderSizeUsd, price: worstPrice },
          { tickSize, negRisk },
          OrderType.FOK
...
async function fetchLeaderTradesSince(leaderAddress: string, sinceTs: number): Promise<LeaderTrade[]> {
...
async function runCopyTradingLoop(): Promise<void> {
  ...
  for (const sub of subscriptions) {
    try {
      ...
      for (const trade of trades) {
        if (trade.timestamp <= sinceTs) continue
        ...
        try {
          ...
          const resp = (await client.createAndPostMarketOrder(
            { tokenID: trade.tokenId, side, amount: effectiveSize, price: worstPrice },
            { tickSize, negRisk },
            OrderType.FOK
          ...
          await recordCopyTrade({
            ...
            amountUsd: effectiveSize,
            status: 'failed',
            errorMessage: (execErr as Error)?.message ?? 'unknown error',
          }).catch(() => {})
