...
function getLocalBuilderConfig(): BuilderConfig {
...
export async function ensureBuilderProxyForWallet(walletId: number): Promise<string> {
  const wallet = await getWalletById(walletId)
  if (!wallet) throw new Error(`wallet ${walletId} not found`)
  if (wallet.type !== 'custodial') throw new Error(`wallet ${walletId} must be custodial`)
  if (!wallet.encrypted_private_key) throw new Error(`wallet ${walletId} missing private key`)

  if (wallet.builder_proxy_address) return wallet.builder_proxy_address
  ...
  try {
    ...
    const statusCode =
      (e as Record<string, unknown>).statusCode as number | undefined ??
      (e as Record<string, unknown>).status as number | undefined ??
      0
