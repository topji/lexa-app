...
export function toInsightRow(market, sample_ts, data, raw) {
    return {
        market,
        sample_ts,
        slug: data.slug ?? null,
...
export async function insertSynthdataInsight(row) {
    const pool = getPool();
    await pool.query(`INSERT INTO synthdata_insights (
...
export async function getLatestSynthdataInsights() {
    const pool = getPool();
    const res = await pool.query(`SELECT market, sample_ts, slug, start_price, current_price, current_outcome,
            synth_probability_up, polymarket_probability_up,
            event_start_time, event_end_time,
            best_bid_price, best_ask_price, best_bid_size, best_ask_size,
            polymarket_last_trade_time, polymarket_last_trade_price, polymarket_last_trade_outcome
