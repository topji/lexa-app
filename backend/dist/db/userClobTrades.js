...
export async function ensureUserClobTradesTable() {
    const pool = getPool();
    await pool.query(`
...
function normalizeClobTrade(t) {
    ...
    if (ts != null) {
        if (ts instanceof Date)
            ...
            tradeTimestamp = new Date(String(ts));
    }
...
export async function upsertClobTrades(walletId, userId, trades) {
    if (trades.length === 0)
        return;
    const pool = getPool();
    for (const t of trades) {
        ...
        await pool.query(`INSERT INTO user_clob_trades (wallet_id, user_id, trade_id, token_id, side, price, size, amount_usd, trade_timestamp, market_slug, raw)
...
export async function listClobTradesByUserId(userId, limit = 100) {
    const pool = getPool();
    const res = await pool.query(`SELECT id, user_id, wallet_id, trade_id, token_id, side, price, size, amount_usd, trade_timestamp, market_slug, raw, created_at
...
export async function getDistinctTokenIdsByUserId(userId) {
    const pool = getPool();
    const res = await pool.query(`SELECT DISTINCT token_id FROM user_clob_trades WHERE user_id = $1 AND token_id IS NOT NULL`, [userId]);
    return res.rows.map((r) => r.token_id);
}
/** Get one market_slug per token_id for this user (for position display). */
export async function getTokenIdToMarketSlug(userId) {
    const pool = getPool();
    const res = await pool.query(`SELECT DISTINCT ON (token_id) token_id, market_slug
     FROM user_clob_trades
     WHERE user_id = $1 AND token_id IS NOT NULL AND market_slug IS NOT NULL
     ORDER BY token_id, trade_timestamp DESC NULLS LAST`, [userId]);
    const map = new Map();
    for (const r of res.rows)
        if (r.market_slug)
            map.set(r.token_id, r.market_slug);
    return map;
}
