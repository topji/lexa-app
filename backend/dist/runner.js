...
import { getLatestOdds } from './db/marketOdds.js';
import { listActiveStrategies } from './db/strategies.js';
import { getWalletById, setWalletClobCreds } from './db/wallets.js';
import { getOpenPositionForStrategy, createOpenPosition, closePosition, countOpenPositionsForUser, } from './db/positions.js';
...
async function getMarketInfoFor(marketCode) {
...
async function ensureWalletClobCreds(walletId) {
...
export async function getClobClientForWallet(walletId) {
    ...
    const signatureType = w.builder_proxy_address ? 2 : w.signature_type;
    const client = new ClobClient(config.clobRestUrl, config.polymarketChainId, signer, creds, signatureType, funder);
...
async function getFunderBalanceAllowance(funderAddress) {
    if (!funderAddress || !utils.isAddress(funderAddress))
        return null;
...
function getAbsChange(latest, side, windowS) {
    const key = `${side}_abs_chg_${windowS}s`;
...
function getPctChange(latest, side, windowS) {
    const key = `${side}_pct_chg_${windowS}s`;
    const raw = latest[key];
    if (raw == null)
        return null;
    const v = toNum(raw);
...
function whyNotEnter(strategy, odds, secondsToExpiry, latest) {
    if (secondsToExpiry == null)
        return 'no seconds_to_expiry';
    if (strategy.market === 'btc-5m') {
        if (secondsToExpiry > 280)
            return 'btc-5m: first 20s blocked';
        if (secondsToExpiry < 30)
            return 'btc-5m: last 30s blocked';
    }
    const currentOdd = strategy.entry_side === 'up' ? odds.up : odds.down;
    if (secondsToExpiry <= strategy.entry_seconds_to_expiry_min)
        return `expiry ${secondsToExpiry}s <= min ${strategy.entry_seconds_to_expiry_min}s`;
    if (strategy.entry_odd_max != null) {
        const entryOddMax = toNum(strategy.entry_odd_max);
        if (currentOdd > entryOddMax)
            return `odd ${currentOdd.toFixed(4)} > max ${entryOddMax}`;
    }
    if (strategy.entry_odd_change_window_s != null) {
        ...
        if (w >= 1 && w <= 5) {
            if (strategy.entry_odd_change_min != null) {
                ...
                if (change == null)
                    return `no abs change history for ${w}s`;
                if (change < toNum(strategy.entry_odd_change_min))
                    return `abs change ${change} < min ${strategy.entry_odd_change_min}`;
            }
            if (strategy.entry_odd_change_pct_min != null) {
                ...
                if (changePct == null)
                    return `no pct change history for ${w}s`;
                // Negative min = "drop by at least X%": allow when changePct <= min (e.g. -2.1 <= -2). Positive = "rise by at least X%": allow when changePct >= min.
                if (minPct <= 0 && changePct > minPct)
                    return `pct change ${changePct} > min ${minPct} (need drop >= ${-minPct}%)`;
                if (minPct > 0 && changePct < minPct)
                    return `pct change ${changePct} < min ${minPct}`;
            }
        }
    }
    return null;
}
function shouldEnter(strategy, odds, secondsToExpiry, latest) {
    return whyNotEnter(strategy, odds, secondsToExpiry, latest) === null;
}
function shouldExit(strategy, positionSide, odds, secondsToExpiry, entryOdd // needed for percentage-based exits
) {
    if (secondsToExpiry == null)
        return { exit: false };
...
export function startRunner() {
    ...
    const walletErrorCounts = new Map();
    const lastEntryByStrategy = new Map();
        ...
        try {
            ...
            for (const s of strategies) {
                ...
                if (!openPos) {
                    ...
                    const expiryKey = latest.expiry_ts instanceof Date ? String(latest.expiry_ts.getTime()) : String(latest.expiry_ts ?? '');
                    ...
                    try {
                        ...
                        if (++globalErrorStreak >= maxGlobalErrors)
                            globalBackoffUntil = Date.now() + globalErrorBackoffMs;
                        continue;
                    }
                    ...
                    try {
                        ...
                        if (funderAddress && wallet?.builder_proxy_address) {
                            ...
                            const rawBal = collateral?.balance;
                            const rawAllowance = collateral?.allowance;
                            const rawAllowances = collateral?.allowances;
                            ...
                            const derivedAllowance = rawAllowance ??
                    ...
                    const resp = (await client.createAndPostMarketOrder({ tokenID, side: Side.BUY, amount: toNum(s.order_size_usd), price: worstPrice }, { tickSize, negRisk }, OrderType.FOK));
                    if (!resp.success) {
                ...
                if (!tokenID || !Number.isFinite(shares) || shares <= 0)
                    continue;
                if ((walletErrorCounts.get(s.wallet_id) ?? 0) >= maxWalletErrors)
                    continue;
                ...
                const tickSize = await client.getTickSize(tokenID);
                const negRisk = await client.getNegRisk(tokenID);
                const resp = (await client.createAndPostMarketOrder({ tokenID, side: Side.SELL, amount: shares, price: toNum(tickSize) }, { tickSize, negRisk }, OrderType.FOK));
                if (!resp.success) {
