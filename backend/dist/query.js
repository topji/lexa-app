...
async function main() {
    ...
    if (windowArg) {
        ...
        result = await pool.query(`SELECT sample_ts, btc_price, up_odd, down_odd,
       ...
       LIMIT $3`, [slug, windowTs, limit]);
    }
    else {
        result = await pool.query(`SELECT window_ts, sample_ts, btc_price, up_odd, down_odd,
         up_pct_chg_1s, up_pct_chg_2s, up_pct_chg_3s, up_pct_chg_4s, up_pct_chg_5s,
         down_pct_chg_1s, down_pct_chg_2s, down_pct_chg_3s, down_pct_chg_4s, down_pct_chg_5s
       FROM btc5m_odds
       WHERE market_slug = $1
