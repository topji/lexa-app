...
const origin_discovery_1 = require("../utils/origin-discovery");
...
class ConnectionManager extends events_1.EventEmitter {
    constructor(config, managerConfig) {
        ...
        this.logger = new logger_1.SDKLogger(config.logging);
    }
    /**
    ...
    async initialize() {
        try {
            ...
            throw new errors_1.MultiConnectionError(`Failed to initialize connections: ${error instanceof Error ? error.message : error}`);
        }
    }
    /**
     ...
     */
    async createConnection(origin, index) {
        ...
        try {
            ...
            this.updateConnectionState(connection, ConnectionState.FAILED, `Connection setup failed: ${error instanceof Error ? error.message : error}`);
            ...
            try {
                ...
                connection.ws.on("error", error => {
                    ...
                    if (connection.state === ConnectionState.CONNECTING) {
                        ...
                        reject(new errors_1.WebSocketError(message));
                    }
                    else {
    ...
    handleConnectionLoss(connection, error) {
        ...
        this.updateConnectionState(connection, ConnectionState.DISCONNECTED, error ? `Connection lost: ${error.message}` : "Connection closed");
        connection.lastError = error;
        // Stop health monitoring
        this.stopHealthMonitoring(connection);
        // Notify status callback
    ...
    scheduleReconnection(connection) {
        ...
        const delay = Math.min(baseDelay * Math.pow(2, connection.reconnectAttempts - 1), constants_1.WS_CONSTANTS.MAX_RECONNECT_INTERVAL);
        this.logger.debug(`Scheduling reconnection for ${connection.id} in ${delay}ms (attempt ${connection.reconnectAttempts})`);
