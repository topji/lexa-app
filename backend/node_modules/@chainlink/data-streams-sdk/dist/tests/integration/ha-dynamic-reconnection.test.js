...
const mockWebSocketServer_1 = require("../utils/mockWebSocketServer");
const metrics_1 = require("../../src/types/metrics");
describe("HA Dynamic Reconnection Integration Tests", () => {
    let mockServer;
    let serverAddress;
    const activeStreams = [];
    beforeEach(async () => {
        mockServer = new mockWebSocketServer_1.MockWebSocketServer();
    ...
    describe("Partial Connection Loss (Production Scenario)", () => {
        it("should handle partial connection drops and attempt reconnection", async () => {
            ...
            const client = (0, index_1.createClient)(config);
        ...
        it("should maintain stream functionality with remaining connections", async () => {
            ...
            await mockServer.broadcast(Buffer.from(JSON.stringify({
                ...mockReport,
                report: {
                    ...mockReport.report,
    ...
    describe("Connection State Transitions (Production Monitoring)", () => {
        it("should accurately track origin states during reconnection cycles", async () => {
            ...
            const connectedOrigins = Object.values(initialMetrics.originStatus).filter(status => status === metrics_1.ConnectionStatus.CONNECTED).length;
            ...
            const disconnectedOrigins = Object.values(disconnectMetrics.originStatus).filter(status => status === metrics_1.ConnectionStatus.DISCONNECTED || status === metrics_1.ConnectionStatus.RECONNECTING).length;
...
//# sourceMappingURL=ha-dynamic-reconnection.test.js.map