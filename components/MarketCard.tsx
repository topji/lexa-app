...
export default function MarketCard({ market }: MarketCardProps) {
  ...
  const formatOutcome = (outcome: { name: string; price: number }, index: number) => {
    ...
    const borderClass = isYes ? 'border-neon-green/30 bg-neon-green/10' : isNo ? 'border-neon-red/30 bg-neon-red/10' : 'border-lexa-border bg-lexa-glass'
    ...
    >
      <div className="mb-4">
        <div className="flex items-start justify-between mb-2">
          ...
          {market.category && (
            <span className="px-2.5 py-0.5 text-xs font-display font-semibold bg-lexa-accent/20 text-lexa-accent rounded-lg whitespace-nowrap ml-2 uppercase tracking-wide">
              {market.category}
            </span>
          )}
        </div>
